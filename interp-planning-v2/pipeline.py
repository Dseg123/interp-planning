...
def train(config: DictConfig) -> Tuple[StateEncoder, np.ndarray, np.ndarray, GridworldProblem, float, dict]:
    ...
    for epoch in range(config.training.num_epochs):
        ...
        if (epoch > 0 and
            ...
            # Learning rate reaches floor by lr_cutoff_pct
            lr_cutoff_epoch = config.training.num_epochs * config.training.lr_cutoff_pct
            lr_progress = min(epoch / lr_cutoff_epoch, 1.0)
            lr_cosine_factor = 0.5 * (1 + np.cos(np.pi * lr_progress))
            current_lr = min_lr + (initial_lr - min_lr) * lr_cosine_factor
            
            # Temperature reaches floor by temp_cutoff_pct
            temp_cutoff_epoch = config.training.num_epochs * config.training.temp_cutoff_pct
            temp_progress = min(epoch / temp_cutoff_epoch, 1.0)
            temp_cosine_factor = 0.5 * (1 + np.cos(np.pi * temp_progress))
            current_temp = min_temp + (initial_temp - min_temp) * temp_cosine_factor
